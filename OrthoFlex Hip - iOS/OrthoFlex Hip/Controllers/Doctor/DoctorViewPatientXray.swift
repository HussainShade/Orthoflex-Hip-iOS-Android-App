...
class DoctorViewPatientXray: UIViewController,UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    
    
    @IBOutlet weak var postXrayBtn: UIButton!
    @IBOutlet weak var preXrayBtn: UIButton!
    ...
    var formPatient = false
    
    override func viewDidLoad() {
        super.viewDidLoad()

        if formPatient {
            preXrayBtn.isHidden = true
            postXrayBtn.isHidden = true
        }else {
            
            preXrayBtn.isHidden = false
            postXrayBtn.isHidden = false
        }
    ...
    func retriveImage() {
        ...
        APIHandler().postAPIValues(type:ImageXray.self, apiUrl: ServiceAPI.retImage+"?id=\(DataManager.shared.patientLoginId)", method: "GET",formData: formData) { [weak self] result in
                 switch result {
                 case .success(let data):
                     print(data)
                     if data.status == "success" {
                         self?.loadImage(url: data.data.preXrayImage, imageView: self?.preOperativeXrayImage)
                         self?.loadImage(url: data.data.postXrayImage, imageView: self?.postpOperativeXrayImage)
                     }else{
